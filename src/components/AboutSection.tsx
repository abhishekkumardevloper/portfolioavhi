...
export default function AboutSection() {
  ...
  return (
    <section id="about" ref={sectionRef} className="section" style={{ padding: "120px 2rem", background: "linear-gradient(180deg, #050505 0%, #080510 50%, #050505 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} style={{ textAlign: "center", marginBottom: 80 }}>
          <span style={{ fontSize: 12, letterSpacing: "0.3em", color: "rgba(14,165,233,0.8)", fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", display: "block", marginBottom: 16 }}>// about me</span>
          <h2 style={{ fontSize: "clamp(36px, 5vw, 64px)", fontWeight: 800, letterSpacing: "-0.02em", fontFamily: "'Space Grotesk', sans-serif", lineHeight: 1.1 }}>
              ...
              <div style={{ position: "absolute", inset: 0, border: "1px solid rgba(14,165,233,0.2)", borderRadius: 24, background: "linear-gradient(135deg, rgba(14,165,233,0.05), rgba(139,92,246,0.05))", backdropFilter: "blur(10px)", overflow: "hidden" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", padding: "0 20px 20px" }}>
                  ...
                  <div style={{ position: "relative", marginBottom: 4 }}>
                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg, rgba(14,165,233,0.4), rgba(139,92,246,0.4))", boxShadow: "0 0 30px rgba(14,165,233,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32 }}>
                      👨‍💻
                    </div>
                  ...
                  {/* Body */}
                  <div style={{ width: 160, height: 220, background: "linear-gradient(to bottom, rgba(14,165,233,0.15), rgba(139,92,246,0.1))", borderRadius: "80px 80px 0 0", border: "1px solid rgba(14,165,233,0.15)" }} />
                </div>
                ...
                {[{ top: 12, left: 12 }, { top: 12, right: 12 }, { bottom: 12, left: 12 }, { bottom: 12, right: 12 }].map((pos, i) => (
                  <div key={i} style={{ position: "absolute", ...pos, width: 20, height: 20, border: "2px solid rgba(6,182,212,0.5)", borderRadius: 2 }} />
                ))}
              </div>

              {/* Floating badges */}
              <div style={{ position: "absolute", top: -20, right: -30, background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)", borderRadius: 12, padding: "8px 16px", backdropFilter: "blur(10px)", animation: "float 5s ease-in-out infinite" }}>
                <span style={{ fontSize: 12, color: "#0ea5e9", fontWeight: 600 }}>🤖 AI Expert</span>
              </div>
              <div style={{ position: "absolute", bottom: 40, left: -40, background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 12, padding: "8px 16px", backdropFilter: "blur(10px)", animation: "float 7s ease-in-out infinite reverse" }}>
            ...
            {cards.map((card, i) => (
              <motion.div key={card.title}
                ...
                style={{
                  ...
                  boxShadow: hoveredCard === i ? "0 20px 60px rgba(14,165,233,0.15)" : "none",
                  backdropFilter: "blur(10px)",
                }}>
                <div style={{ fontSize: 28, marginBottom: 10 }}>{card.icon}</div>
                <div style={{ fontSize: 11, letterSpacing: "0.15em", color: "rgba(14,165,233,0.7)", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace", marginBottom: 4 }}>{card.title}</div>
                <div style={{ fontSize: 20, fontWeight: 700, color: "white", marginBottom: 6, fontFamily: "'Space Grotesk', sans-serif" }}>{card.value}</div>
                <p style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", lineHeight: 1.6 }}>{card.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
